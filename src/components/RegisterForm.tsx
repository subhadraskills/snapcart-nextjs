...
function RegisterForm({previousStep}:propType){
    ...
    return(
        ...
        px-6 py-10 bg-white relative'>
            ...
            text-green-700 hover:text-gray-800 transition-colors cursor-pointer'
...
transition={{
 ...
 <div className="relative">
    ...
    />
        ...
        cursor-pointer" onClick={()=>setShowpassword(false)}/>:<EyeIcon className="absolute right-3 top-3.5 w-5 h-5 text-gray-500 cursor-pointer" onClick={()=>setShowpassword(true)}/>
    }
  ...
  <span className="flex-1 h-px bg-gray-200"></span>
    ...
    <div className="w-full flex items-center justify-center gap-3 border border-gray-300 hover:bg-gray-50 py-3 rounded-xl text-gray-700 font-medium transition-all duration-200 " onClick={()=>signIn("google",{callbackUrl:"/"})}>
        ...
        Continue with Google
            ...
            <p className="cursor-pointer text-gray-600 text-sm flex items-center gap-1 " onClick={()=>router.push("/login")}>Already have an 
                 account ? <LogIn className="w-4 h-4"/>
                 <span className="text-green-600">Sign in</span></p>
