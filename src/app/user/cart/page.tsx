...
import { decreaseQuantity, increaseQuantity, removeFormCart } from '@/redux/cartSlice'
            ...
            <div className='lg:col-span-2 space-y-5'>
                <AnimatePresence>
                        ...
                        {cartData.map((item,index)=>(
                            ...
                            >
                                <div className='relative w-28 h-28 sm:h-24 md:w-28 md:h-28 flex-shrink-0
                                ...
                                bg-gray-50 px-3 py-2 rounded-full'>
                                  ...
                                  border border-gray-200 ' onClick={()=>dispactch(decreaseQuantity(item._id))}><Minus size={14} className="text-green-700"/></button>

                                  <span className='font-semibold text-gray-800 w-6 text-center'>{item.quantity}</span>

                                  <button className='bg-white p-1.5 rounded-full hover:bg-green-100 transition-all
                                  border border-gray-200' onClick={()=>dispactch(increaseQuantity(item._id))}><Plus size={14} className="text-green-700"/></button>

                                </div>
                               <button className='sm:ml-4 mt-3 sm:mt-0 text-red-500 hover:text-red-700
                               transition-all' onClick={()=>dispactch(removeFormCart(item._id))}><Trash2 size={18}/></button>
                ...
                >
                  ...
                  hover:bg-green-700 transition-all font-semibold text-sm sm:text-base' onClick={()=>router.push("/user/checkout")}>
                    Proceed to Checkout
